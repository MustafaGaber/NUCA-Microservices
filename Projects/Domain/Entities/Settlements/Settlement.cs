...
namespace NUCA.Projects.Domain.Entities.Settlements
{
    public class Settlement : AggregateRoot
    {
        ...
        private readonly List<SettlementWithholding> _withholdings = new();
        public virtual IReadOnlyList<SettlementWithholding> Withholdings => _withholdings.ToList();
        ...
        public double TotalDue => CurrentWorksAndSupplies + ServiceTax + SupervisionCommission;
             ...
             double contractPaperPrice)
        {
            ...
            Submitted = false;
            _withholdings.AddRange(statement.Withholdings.Select(w => new SettlementWithholding(w.Name, w.Value, w.Type, true)));
            Update(
        ...
        public void AddWithholding(SettlementWithholding withholding)
        {
            if (Submitted)
            {
                throw new InvalidOperationException();
        ...
        public void UpdateWithholding(long withholdingId, SettlementWithholding withholding)
        {
            if (Submitted)
            {
                throw new InvalidOperationException();
            }
            SettlementWithholding? oldWithholding = _withholdings.Find(d => d.Id == withholdingId);
        ...
        public void RemoveWithholding(long id)
        {
            ...
            SettlementWithholding? withholding = _withholdings.Find(d => d.Id == id);
        ...
        {
            NetAmount = TotalDue - TotalWithholdings;
        }
